...
软件实现：通过手动翻转电平来实现时序，再拼接时序实现完整的时序。
...
- 未被选中的从机的MISO引脚必须设置为高阻态（防止一条线多个输出导致电平的冲突）。
- SCK完全由主机控制。
...
1、时序基本单元——起始、终止条件，SS低电平期间就代表正在通信：

![](img/19.基本单元1.png)

CPOL——Clock Polarity，时钟极性；CPHA——Clock Phase，时钟相位；SPI可配置的位，用于配置SPI模式，共有四种配置方式，实际使用中学习一种即可，功能都是一样的。

2、时序基本单元——交换一个字节（模式0，应用最多）：（CPOL——Clock Polarity，时钟极性；CPHA——Clock Phase，时钟相位）
